...
function flatToolItems(toolItems) {
    return toolItems.reduce((preVal, curVal) => [...preVal, ...curVal.componentList], []);
}
...
function viewSchemaMatch(target, toolItem) {
    ...
    return (
        schemaIncludes(target, baseViewSchema) &&
                ...
                if (emptyComponentValue[curVal]) {
                    ...
                    if (schemaOptions) {
                        ...
                        for (const k in schemaOptions) {
                            if (Object.prototype.hasOwnProperty.call(schemaOptions, k)) {
                    ...
                    if (uiOptions) {
                        ...
                        for (const key in uiOptions) {
                            if (Object.prototype.hasOwnProperty.call(uiOptions, key)) {
                                const tmpVal = curSchemaUiOptions[key];
                                if (tmpVal !== undefined) {
                                    preVal[curVal].uiOptions[key] = tmpVal;
                                }
                            }
                        }
                    }
...
export default function jsonSchema2ComponentList(code, toolItems) {
    ...
    const {schema, uiSchema, ...formConfig} = data;
    ...
    const getChildList = curSchema =>
        (curSchema.$$parentEditorItem && curSchema.$$parentEditorItem.childList) || componentList;
