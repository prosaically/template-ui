...
export function generateEditorItem(toolItem) {
    const {propsSchema, viewSchema} = toolItem.componentPack;
    const id = "field_" + letters(7);
    ...
    return {
        ...
        id,
        ...(viewSchema.properties || (viewSchema.items && viewSchema.items.properties) ? {childList: []} : {})
    };
}
...
export function editorItem2SchemaFieldProps(editorItem, formData) {
    // baseValue
    const {schemaOptions: baseSchemaOptions, uiOptions: {required, ...baseUiOptions} = {}} =
    ...
    // rules
    const {schemaOptions: ruleSchemaOptions, uiOptions: ruleUiOptions} = editorItem.componentValue.rules || {};
